  ...
  copy: {
    ...
    zh: {
      ...
      blocks: [
        ...
        {
          t: 'p',
          text: '索引与模型下载通过标为 polite 的实时区域报告进度，播报会等到停顿再发生，而不是打断你。进度文字会说出当前阶段——解析、分块、向量化、建索引、加载模型——在有计数时给出已完成数与总数。模型下载同样以已加载 MB 对总 MB 报告。错误出现在以 alert 播报的区域里，并带一个带标签的关闭按钮。可选生成模型下载前的确认是一个标为模态的对话框，里面写明体积，并有两个按钮：确认与取消。',
        },
        { t: 'h2', text: '目前做得不够的地方' },
        {
          t: 'p',
